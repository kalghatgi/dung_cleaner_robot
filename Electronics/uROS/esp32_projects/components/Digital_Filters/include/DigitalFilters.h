...
public:
	...
	LowPassFilter(double idt, double omega_c, double ioutput = 0):
		...
		output(ioutput){
			if(omega_c < idt){
				ESP_LOGE("LowPassFilter constructor error: tua_c is smaller than the sample time dt.", "");
			}
		}
	/**
	...
	LowPassFilter3MatchedZ(long double sampleTime, long double omega_c):
		...
		}
		{
			if(omega_c / (2 * M_PI) < sampleTime){
				ESP_LOGE("LowPassFilter3MatchedZ constructor error: tua_c is smaller than the sample time dt.", "");
			}
		}
	double update(double newValue) final{
